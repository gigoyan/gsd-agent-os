...
3. Use [`.planning/templates/quick-task-template.md`](../../../.planning/templates/quick-task-template.md) as a lightweight planning aid if the task needs a short written record.
4. Define the smallest meaningful behavior slice and the targeted test or check to update first. If the task depends on prior durable context, request a narrow `gsd-memory-lookup` context pack first. If test-first is impractical, record the reason and nearest safeguard before implementation.
5. Update the targeted test or check first when practical, run proportional validation, implement the minimum change needed, and log the outcome in [`.planning/STATE.md`](../../../.planning/STATE.md). If the task produces durable insight, record the later `gsd-session-save` follow-up as `candidate` or `none` rather than writing durable memory here.
6. If the task expands, stop and redirect to `$gsd-plan-milestone`.
