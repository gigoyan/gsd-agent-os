...
2. Apply [`.planning/templates/delegated-agent-contract.md`](../../../.planning/templates/delegated-agent-contract.md) to all child prompts, child lifecycle handling, and child-output review.
3. Determine the correct immediate delegated action from state and artifacts:
...
4. For each active phase, use at most two children in sequence:
...
5. Choose the child role from the immediate delegated action plus the current phase domain and selected stack context already established by repo-local planning artifacts. Keep that role choice root-owned, stack-agnostic at the blueprint level, and constrained to the assigned planning, execution, or verification-and-next-phase-planning child step.
6. Read `.planning/CONTEXT_INDEX.md` when available and pass only the relevant context-routing section, routing row, module card, or phase `Context Routing` summary to the delegated child. Do not pass the full repository map unless the delegated step genuinely requires it.
7. Read the active milestone and phase `Source Traceability` sections. When they cite `.planning/source-materials/SOURCE_MATERIALS.md`, follow [`.planning/templates/source-materials-contract.md`](../../../.planning/templates/source-materials-contract.md) and pass only the relevant `source_id`, claim IDs, anchors, evidence statuses, and registry path to execution or verification children.
8. Resolve child spawn settings for the current run before spawning.
...
9. Spawn exactly one bounded sub-agent for the next child action. The root session should pass only the minimal prompt and required file references, use the resolved child spawn settings, wait for the result, and not perform the delegated work itself. If the root session needs prior durable context to route correctly, it may request a narrow `gsd-memory-lookup` context pack before spawning, but it still remains the only orchestrator.
10. After a child returns, the root orchestrator must read the result, close the completed child with `close_agent`, and only then spawn the next child if the milestone should continue. Route only from explicit response signals:
   - `Phase Status`
   - `Milestone Status`
   - `Next-Step Prompt`
11. Continue spawning the next child action while `Milestone Status` is `in_progress`: execution for the active phase, then verification-and-next-phase-planning for that same phase. If a child result or stale state requires durable context to route safely, refresh the narrow memory context pack before the next loop; do not infer routing from memory alone.
12. Stop only when a child result explicitly reports `Milestone Status: completed`, or when the child result says no automatic next-step prompt applies and the next action is genuinely unclear.

## Rules
- This skill requires an explicit user request for subagent-driven milestone automation.
- Keep the root session as manager and follow the delegated-agent contract. Do not let child agents recursively fan out unless the environment is intentionally configured for deeper delegation.
...
- When milestone or phase source traceability is present, child prompts should include only the relevant registry path, `source_id`, claim IDs, anchors, and evidence statuses needed for the assigned execution or verification step under [`.planning/templates/source-materials-contract.md`](../../../.planning/templates/source-materials-contract.md).
- Do not tell child agents to broad-scan raw source-material folders. Source-material consumption during execution and verification must come from prepared phase or milestone traceability and cited registry entries.
...
- Apply [`.planning/templates/vault-operating-spec.md`](../../../.planning/templates/vault-operating-spec.md) for vault-memory boundaries during orchestration: repo artifacts control routing, vault lookup is durable context only, retrieval stays in `projects/<vault-project-id>/`, sibling namespaces and shared-root durable writes are prohibited, and durable writeback routes through `gsd-session-save`.
...
Every delegated child prompt must follow [`.planning/templates/delegated-agent-contract.md`](../../../.planning/templates/delegated-agent-contract.md) and make the assigned child action explicit: planning, execution, or the explicit `$gsd-run-milestone` verification-and-next-phase-planning composite step.
